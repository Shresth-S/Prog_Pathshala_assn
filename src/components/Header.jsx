...
export default function Header() {
  return (
    ...
    //   </div>
    // </header>
    <header className='bg-bgNeutralSilver h-20 w-full'>
    <div className='flex justify-between items-center max-w-6xl mx-auto p-3'>
      <Link to='/'>
        <h1 className='font-bold text-sm sm:text-xl flex items-center gap-2'>
          <img src="/logo.png" alt="x" className='h-6 w-8 mt-1'/>
          <span className='text-slatedark sm:inline font-bold text-sm sm:text-xl'>Nexcent</span>
        </h1>
      </Link>

      <ul className='hidden md:flex h-6 mt-1 gap-10 font-normal text-base text-slatedark'>
      ...
      <ul className='gap-0 flex h-10 mt-1.5'>
        ...
        <Link to='/signup'>
          <button className='bg-myGreen text-myWhite rounded-md py-2 px-5 w-24 text-sm font-medium text-center hover:opacity-95'>
            Sign up
          </button>
        </Link>
      </ul>
